...
class DeepdriveDatasetWriter(object):
    ...
    def __init__(self):
        self.input_path = os.path.join(os.getcwd(), 'bdd100k')
    ...
    def get_image_label_folder(self, fold_type=None, version=None):
        ...
        full_labels_path = os.path.join(expansion_labels_folder)
        full_images_path = os.path.join(expansion_images_folder)
...
#             full_labels_path = os.path.join(full_labels_path, '..')
        ...
        # print(full_images_path, full_labels_path, valid_folder_structure_new_format)
    ...
    def _get_boundingboxes_new_format(self, annotations_for_picture_id,image_path):
        ...
        for obj in annotations_for_picture_id['labels']:
            if 'box2d' not in obj:
                continue
            boxid.append(obj['id'])
            xmin.append(obj['box2d']['x1']/image_width)
            xmax.append(obj['box2d']['x2']/image_width)
            ymin.append(obj['box2d']['y1']/image_height)
            ymax.append(obj['box2d']['y2']/image_height)
            label.append(obj['category'])
    ...
    def _get_tf_feature_dict(self, image_id, image_path, image_format, annotations, new_format=True):
        if not new_format:
            ...
            boxid, xmin, xmax, ymin, ymax, label_id, label, truncated, occluded = \
                self._get_boundingboxes_new_format(annotations,image_path)
